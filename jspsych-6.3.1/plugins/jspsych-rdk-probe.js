...
jsPsych.plugins["rdk"] = (function() {
	...
	plugin.info = {
	    ...
	    parameters: {
			...
			time_to_sound: {
				type: jsPsych.plugins.parameterType.INT,
				pretty_name: "Time to sound",
				default: 200,
				description: "The time before the sound is initialized"
			},
			play_sound: {
				type: jsPsych.plugins.parameterType.BOOL,
				pretty_name: "Play sound",
				default: false,
				description: "Whether or not a sound should be played"
			},
		    response_ends_trial: {
		      type: jsPsych.plugins.parameterType.BOOL,
	...
	plugin.trial = function(display_element, trial) {
		...
		trial.play_sound = assignParameterValue(trial.play_sound, false);
		trial.time_to_sound = assignParameterValue(trial.time_to_sound, 200);
		...
		*/
		...
		function end_trial() {
			...
			var trial_data = { 
				...
				time_to_sound: trial.time_to_sound, //The time to tone
				play_sound: trial.play_sound,//Whether or not to play sound
			...
			function animate() {
				...
				//Else continue with another frame request
					...
					if ( (!timerHasStarted) && (trial.trial_duration > 0) ){
						...
						if (trial.play_sound){
							soundID = window.setTimeout(play_audio,trial.time_to_sound)
						}
